...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['item7'])) $selectedItems[] = $_POST['item7'];
        if (isset($_POST['item8'])) $selectedItems[] = $_POST['item8'];


        // Loop through selected items
        foreach ($selectedItems as $item) {
            // Fetch data from academic building/crc
            $sql = "SELECT `Type of Plastic`, Weight, Recyclable, Treatment FROM `all departments` WHERE Name = :item_name";
